...
export function ChatStage({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="relative flex h-full min-h-0 flex-col bg-[#080a0f]">
      ...
      <div className="relative z-10 flex h-full min-h-0 flex-col"><div className="px-2 pt-2 sm:px-4 sm:pt-3"><KairoGlassPanel /></div>{children}<div className="flex items-center justify-center gap-2 px-2 sm:px-4"><KairoCommandSandbox /><KairoLibrary /></div></div>
    </div>
  );
}
