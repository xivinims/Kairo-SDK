...
import { loadKairoSkills } from "@/lib/kairo-skills";
import { useApp } from "@/lib/store";

export function KairoGlassPanel() {
  const draft = useApp((state) => state.draft);
  const sending = useApp((state) => state.sending);
  const skills = loadKairoSkills(draft || "conversa geral").slice(0, 4);
  const steps = sending ? ["Entendendo pedido", "Executando skills", "Verificando resultado"] : ["Skills carregadas sob demanda", "Categorias detectadas", "Pronto para validar"];
  ...
  return (
    <section aria-label="Kairo Agent" className="kairo-glass-panel relative mx-auto mb-3 w-full max-w-2xl overflow-hidden rounded-[28px] border border-white/[0.14] bg-white/[0.07] p-3 text-white shadow-[0_24px_80px_rgba(0,0,0,0.3)] backdrop-blur-2xl backdrop-saturate-150">
      ...
      <div className="relative flex items-center gap-2"><div className="flex size-9 shrink-0 items-center justify-center rounded-full border border-white/15 bg-white/10"><WandSparkles className="size-[17px] text-cyan-100" /></div><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><span className="text-[13px] font-semibold">Kairo Agent</span><span className="inline-flex items-center gap-1 rounded-full border border-emerald-300/20 bg-emerald-300/10 px-2 py-0.5 text-[10px] text-emerald-100"><span className="size-1.5 rounded-full bg-emerald-300" /> online</span></div><p className="truncate text-[11px] text-white/50">Skills carregadas conforme a tarefa</p></div><button type="button" aria-label="Abrir opções" className="press flex size-8 items-center justify-center rounded-full border border-white/10 bg-white/[0.07] text-white/70 transition hover:bg-white/[0.14]"><ChevronDown className="size-4" /></button></div>
      <div className="relative mt-3 flex items-center gap-2 overflow-x-auto no-scrollbar"><div className="flex shrink-0 items-center gap-1.5 rounded-full border border-white/10 bg-black/15 px-2.5 py-1.5 text-[11px] text-white/65"><BrainCircuit className="size-3.5 text-cyan-200" />{sending ? "verificando" : "dupla verificação"}</div>{skills.map((skill) => <span key={skill.id} title={skill.description} className="shrink-0 rounded-full border border-white/10 bg-white/[0.08] px-2.5 py-1.5 text-[11px] text-white/70">{skill.label}</span>)}<Sparkles className="ml-auto size-4 shrink-0 text-violet-200/80" /></div>
      ...
      <div className="relative mt-2 flex items-center gap-1 text-[10px] text-white/35"><ListChecks className="size-3.5" /> Resumo operacional; a cadeia privada do modelo não é exposta.</div>
    </section>
  );
}
