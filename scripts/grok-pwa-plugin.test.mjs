...
const TEST_CLEAN_CWD = mkdtempSync(join(tmpdir(), "grok-test-isolated-cwd-"));

function injectGrokPwaHead(html, ctx = {}) {
  return rawInjectGrokPwaHead(html, { cwd: TEST_CLEAN_CWD, ...ctx });
}

function createHeadInjector(ctx = {}) {
  return rawCreateHeadInjector({ cwd: TEST_CLEAN_CWD, ...ctx });
}
